---
title: "How Styling and Theming Work in PrismUI Native"
description: "Why PrismUI Native ships real CSS files for React Native, how its design tokens derive states automatically, and how Pro themes change components without forking them."
date: "2026-07-31"
...
image: "https://prismui-assets.nyc3.cdn.digitaloceanspaces.com/blog/prismui-native-styling.jpg"
---
...
Notably, this does not go through a React theme provider. Uniwind resolves the variables, so a theme change does not cascade a context update down your entire tree. `PrismUINativeProvider` exists, but it handles safe-area insets, text defaults, the toast host, and RTL — it is app infrastructure, not a theme engine.

## The Pro Theme Pattern

Because tokens are variables and component styles are CSS classes, a complete visual identity can be packaged as a stylesheet. That is how PrismUI Native Pro ships themes, and it works in two passes. The Brutalism theme shows both clearly, since it is about as far from the default look as a theme can get.
...
```css
@import "prismui-native/styles";
@import "prismui-native-pro/styles";
@import "prismui-native-pro/themes/brutalism";
```
...
- **The vocabulary matches the rest of PrismUI.** Semantic roles, foreground pairing, and compound components are the same ideas you already use on the web.
