...
For a complete reference, see: [Colors Documentation](/docs/native/getting-started/colors), [Default Theme Variables](https://github.com/khulnasoft/heroui-native/blob/main/src/styles/variables.css), [Shared Theme Utilities](https://github.com/khulnasoft/heroui-native/blob/main/src/styles/theme.css)

**Calculated variables (Tailwind):**

We use Tailwind's `@theme` directive to automatically create calculated variables for hover (pressed) states and radius variants. These are defined in [theme.css](https://github.com/khulnasoft/heroui-native/blob/main/src/styles/theme.css):
...
By default, PrismUI Native uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast on soft backgrounds. If you prefer more saturated, vibrant soft foreground colors, import the optional `heroui-native/styles/vibrant` stylesheet alongside the base styles:
