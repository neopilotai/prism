...
PrismUI Native v1.0.8 adds right-to-left layout support across the library. Component styles move from physical `left`/`right` to Yoga logical properties, and the JS-driven logic Yoga cannot cover — gesture deltas, animation offsets, popover alignment — now resolves direction through a new `isRTL` provider config, with a `LayoutDirectionScope` escape hatch for subtrees. This release also makes `/native/styles` declare its own `@source`, so class scanning no longer depends on a hardcoded `node_modules` path in your app.
...
`/native/styles` now points Tailwind at its own files:
