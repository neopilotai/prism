...
<Callout type="info">
  **No migration required.** Existing `import { Text } from "prismui-native"` continues to work via deprecated re-exports. Migrate to `Typography` at your convenience before a future major release removes the deprecated aliases.
</Callout>

**Related PR:** [#417](https://github.com/khulnasoft/prismui-native/pull/417)

### Vibrant Theme Palette (Optional)

A new optional `prismui-native/styles/vibrant` stylesheet is available for apps that prefer more saturated brand colors on soft variants. It keeps the readability improvements from the refined soft-foreground tokens while preserving high color saturation for icons and text on soft backgrounds.
...
import "prismui-native/styles";
import "prismui-native/styles/vibrant";
```

Import `prismui-native/styles/vibrant` after the base styles to override soft-foreground tokens with the more saturated values. No component code changes are required to opt in.

**Related PR:** [#420](https://github.com/khulnasoft/prismui-native/pull/420)
...
- A new optional `prismui-native/styles/vibrant` export preserves saturated brand colors on soft variants for apps that want a more vivid look
...
**Related PR:** [#421](https://github.com/khulnasoft/prismui-native/pull/421)
...
The peer dependency range for `@gorhom/bottom-sheet` has been bumped from `^5.2.8` to `^5.2.9`. Apps that use PrismUI Native's [BottomSheet](/docs/native/components/bottom-sheet) component (or any of `Menu` / `Popover` / `Select` with `presentation="bottom-sheet"`) should ensure they are on a compatible version:
...
- Example app slug/bundle renamed to `prismui-native-oss`; Android `WithStateToggle` padding adjusted
...
import { Text, textClassNames, type TextProps } from "prismui-native";
