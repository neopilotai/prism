...
<Callout type="info">
  **No migration required.** Existing `import { Text } from "/native"` continues to work via deprecated re-exports. Migrate to `Typography` at your convenience before a future major release removes the deprecated aliases.
</Callout>
...
A new optional `/native/styles/vibrant` stylesheet is available for apps that prefer more saturated brand colors on soft variants. It keeps the readability improvements from the refined soft-foreground tokens while preserving high color saturation for icons and text on soft backgrounds.
...
import "/native/styles";
import "/native/styles/vibrant";
```

Import `/native/styles/vibrant` after the base styles to override soft-foreground tokens with the more saturated values. No component code changes are required to opt in.
...
- A new optional `/native/styles/vibrant` export preserves saturated brand colors on soft variants for apps that want a more vivid look
...
- Example app slug/bundle renamed to `/native-oss`; Android `WithStateToggle` padding adjusted
...
import { Text, textClassNames, type TextProps } from "/native";
