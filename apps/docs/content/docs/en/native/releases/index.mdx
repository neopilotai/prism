...
This release adds right-to-left layout support across the library: component styles move to Yoga logical properties, and direction-sensitive behavior in Slider, Skeleton, Switch, Tabs, InputGroup, Popover, and Menu resolves through a new `isRTL` provider config with a `LayoutDirectionScope` escape hatch for subtrees. It also makes `/native/styles` declare its own `@source`, so class scanning no longer depends on a hardcoded `node_modules` path. Card.Header and TagGroup.List change how they align children, both with one-line overrides.
...
🎉 A new CLI for scaffolding a PrismUI Native project in one command. `npx create-/native-app@latest my-app` generates a complete Expo Router app with PrismUI Native, Uniwind, Tailwind CSS, every required peer dependency, and the provider wiring already in place — pick between a single-screen or tabs starter template.

[Read full release notes →](/docs/native/releases/create-/native-app)
