---
title: All Releases
description: All updates and changes to PrismUI Native, including new features, fixes, and breaking changes.
---
...
This release adds right-to-left layout support across the library: component styles move to Yoga logical properties, and direction-sensitive behavior in Slider, Skeleton, Switch, Tabs, InputGroup, Popover, and Menu resolves through a new `isRTL` provider config with a `LayoutDirectionScope` escape hatch for subtrees. It also makes `prismui-native/styles` declare its own `@source`, so class scanning no longer depends on a hardcoded `node_modules` path. Card.Header and TagGroup.List change how they align children, both with one-line overrides.
...
🎉 A new CLI for scaffolding a PrismUI Native project in one command. `npx create-prismui-native-app@latest my-app` generates a complete Expo Router app with PrismUI Native, Uniwind, Tailwind CSS, every required peer dependency, and the provider wiring already in place — pick between a single-screen or tabs starter template.

[Read full release notes →](/docs/native/releases/create-prismui-native-app)
...
🎉 PrismUI Native reaches its first stable release, graduating from beta and release candidate stages. This milestone includes the new LinkButton component, sub-menu conflict resolution, an optional `@gorhom/bottom-sheet` peer dependency, and improved type safety for `useThemeColor`.
...
This release introduces the Alert compound component with five status variants and accessibility primitives, extracts Radio into a standalone component with dual-mode operation, and adds the animated Select.TriggerIndicator subcomponent. It also delivers PrismUINativeProviderRaw for bundle optimization, a `disableFullWindowOverlay` prop for iOS debugging, unified `styles` prop support across six components, and theme surface variable refactoring with explicit per-theme definitions.
...
PrismUI Native follows a regular release cycle:
...
Found an issue or want to contribute? Check out our [GitHub repository](https://github.com/khulnasoft/prismui-native).
