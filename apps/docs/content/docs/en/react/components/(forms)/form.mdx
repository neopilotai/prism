...
```tsx
import { Form } from '@khulnasoft/react';
```
...
```tsx
import {Form, Button} from '@khulnasoft/react';
...
`Form` renders a native `<form>` element and focuses on validation and submission. It does not include dedicated BEM classes in `@khulnasoft/styles`—apply container styles from global CSS, and use field components for control-level customization.
...
PrismUI follows the [BEM](https://getbem.com/) methodology for components that ship styles in `@khulnasoft/styles`.
...
The Form component works seamlessly with PrismUI's form field components:
