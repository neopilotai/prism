...
```tsx
import { InputOTP } from '@khulnasoft/react';
```
...
Use the `pattern` prop to restrict input to specific characters. PrismUI exports common patterns like `REGEXP_ONLY_CHARS` and `REGEXP_ONLY_DIGITS`.
...
The InputOTP component uses these CSS classes ([View source styles](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/input-otp.css)):
...
PrismUI re-exports common regex patterns from input-otp for convenience:

```tsx
import { REGEXP_ONLY_DIGITS, REGEXP_ONLY_CHARS, REGEXP_ONLY_DIGITS_AND_CHARS } from '@khulnasoft/react';
