...
```tsx
import { TextField } from '@prismui/react';
```
...
```tsx
import {TextField, Label, Input, Description, FieldError} from '@prismui/react';
...
- **Input** - Single-line text input from `@prismui/react`
- **TextArea** - Multi-line text input from `@prismui/react`
