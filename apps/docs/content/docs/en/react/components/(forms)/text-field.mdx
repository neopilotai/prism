...
```tsx
import { TextField } from '@khulnasoft/react';
```
...
```tsx
import {TextField, Label, Input, Description, FieldError} from '@khulnasoft/react';
...
- **Input** - Single-line text input from `@khulnasoft/react`
- **TextArea** - Multi-line text input from `@khulnasoft/react`
