...
```tsx
import { DateField } from '@prismui/react';
```
...
```tsx
import {DateField, Label, Description, FieldError} from '@prismui/react';
...
- **DateField.Input** - Input component with segmented editing from `@prismui/react`
