...
```tsx
import { DateField } from '@khulnasoft/react';
```
...
```tsx
import {DateField, Label, Description, FieldError} from '@khulnasoft/react';
...
- **DateField.Input** - Input component with segmented editing from `@khulnasoft/react`
