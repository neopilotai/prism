...
```tsx
import { TimeField } from '@prismui/react';
```
...
```tsx
import {TimeField, Label, Description, FieldError} from '@prismui/react';
...
- **TimeField.Input** - Input component with segmented editing from `@prismui/react`
