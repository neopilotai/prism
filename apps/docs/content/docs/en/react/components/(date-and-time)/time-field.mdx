...
```tsx
import { TimeField } from '@khulnasoft/react';
```
...
```tsx
import {TimeField, Label, Description, FieldError} from '@khulnasoft/react';
...
- **TimeField.Input** - Input component with segmented editing from `@khulnasoft/react`
