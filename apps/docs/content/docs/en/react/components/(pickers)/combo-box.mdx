...
```tsx
import { ComboBox } from '@prismui/react';
```
...
```tsx
import { ComboBox, Input, Label, Description, Header, ListBox, Separator } from '@prismui/react';
...
The ComboBox component uses these CSS classes ([View source styles](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/combo-box.css)):
...
```tsx
import { ComboBox, Input, Label, ListBox, Header, Separator } from '@prismui/react';
...
```tsx
import { Collection, ComboBox, EmptyState, Input, Label, ListBox, ListBoxLoadMoreItem, Spinner } from '@prismui/react';
...
```tsx
import { ComboBox, FieldError, Input, Label, ListBox } from '@prismui/react';
