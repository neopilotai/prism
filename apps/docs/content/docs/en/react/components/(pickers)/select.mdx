...
```tsx
import { Select } from "@khulnasoft/react";
```
...
```tsx
import {Select, Label, Description, Header, ListBox, Separator} from "@khulnasoft/react";
...
The Select component uses these CSS classes ([View source styles](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/select.css)):
