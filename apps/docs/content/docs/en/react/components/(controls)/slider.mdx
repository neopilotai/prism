...
```tsx
import { Slider } from '@khulnasoft/react';
```
...
```tsx
import { Slider, Label } from '@khulnasoft/react';
...
The Slider component uses these CSS classes ([View source styles](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/slider.css)):
