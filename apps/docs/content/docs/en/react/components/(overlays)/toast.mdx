...
```tsx
import { Toast, toast } from '@khulnasoft/react';
```
...
```tsx
import { Toast, Button, toast } from '@khulnasoft/react';
...
The Toast component uses these CSS classes ([View source styles](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/toast.css)):
