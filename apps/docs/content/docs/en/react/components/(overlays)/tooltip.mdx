...
```tsx
import { Tooltip } from '@khulnasoft/react';
```
...
```tsx
import { Tooltip, Button } from '@khulnasoft/react';
...
The Tooltip component uses these CSS classes ([View source styles](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/tooltip.css)):
