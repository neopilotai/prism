...
**Styles separated from implementation:** `@khulnasoft/styles` is standalone CSS. `@prismui/react` handles behavior. Use the styles with React, plain HTML + Tailwind, or any framework. BEM class names make every slot customizable globally. Swap themes to change not just variables, but how components look and feel.
...
```tsx
// Root import
import { Button, Card, Table } from "@prismui/react";

// Subpath import
import { Button } from "@prismui/react/button";
import { Card } from "@prismui/react/card";
import { Table } from "@prismui/react/table";
```
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  ...
  <Tab value="pnpm">
    ```bash
    pnpm add @khulnasoft/styles @prismui/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @khulnasoft/styles @prismui/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @khulnasoft/styles @prismui/react
    ```
  </Tab>
</Tabs>
