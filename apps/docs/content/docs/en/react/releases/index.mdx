...
Patch release: `@prismui/react` no longer bundles its own copy of `react-aria`. The `react-aria` subpaths are externalized in the Rollup build, so consumers resolve to the single `react-aria` installed in their project instead of a vendored copy. Also fixes `SwitchGroup` horizontal layout.
...
<Callout type="warning">
  **Critical Build Issue Fixed**: This version (beta.4) had a critical build issue that has been fixed in **beta.5**. Please upgrade to `@khulnasoft/styles@3.0.0-beta.5` and `@prismui/react@3.0.0-beta.5` to ensure proper TypeScript declaration generation and export resolution.
</Callout>
