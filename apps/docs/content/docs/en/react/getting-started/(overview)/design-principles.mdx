...
```tsx
import { Tabs, TabList, Tab, TabPanel } from '@prismui/react';
...
import { Alert, AlertIcon, CardHeader, AccordionTrigger } from '@prismui/react';
...
Styles (`@khulnasoft/styles`) are separate from logic (`@prismui/react`), enabling use with any framework or vanilla HTML. See [Tailwind Play example](https://play.tailwindcss.com/vMYXzKPyUx).
