---
title: Styling
description: Style PrismUI components with CSS, Tailwind, or CSS-in-JS
icon: brush
links:
  themes: true
  tailwind: theme
...
PrismUI components provide flexible styling options: Tailwind CSS utilities, CSS with [BEM](https://getbem.com/) classes or data attributes, CSS-in-JS libraries, and render props for dynamic styling.

## Basic Styling

**Using className:** All PrismUI components accept `className` props:
...
PrismUI scroll slots use `@apply scrollbar` in component CSS. For your own overflow containers, use the utilities from `@khulnasoft/styles`:
...
| Utility | Effect |
|---------|--------|
| `scrollbar` | PrismUI thumb (reads theme `--scrollbar-*` variables) |
| `scrollbar-thin` | PrismUI themed thin scrollbar |
...
PrismUI components expose their state through data attributes, similar to CSS pseudo-classes:
...
PrismUI uses [BEM methodology](https://getbem.com/) for consistent class naming:
...
```tsx
import { Button as HeroButton, type ButtonProps } from '@khulnasoft/react';
import { buttonVariants, tv, type VariantProps } from '@khulnasoft/styles';
...
View all component classes in [@khulnasoft/styles/components](https://github.com/khulnasoft/prismui/tree/main/packages/styles/components).
