---
title: Theming
description: Customize PrismUI's design system with CSS variables and global styles
icon: palette
links:
  themes: true
  tailwind: theme
...
PrismUI uses CSS variables and [BEM](https://getbem.com/) classes for theming. Customize everything from colors to component styles using standard CSS.
...
> **Dark mode**: For a complete setup guide with `next-themes` and PrismUI's `useTheme` hook, see [Dark Mode](/docs/handbook/dark-mode).
...
@import "@khulnasoft/styles/base/base.css" layer(base);

/* Theme variables */
@import "@khulnasoft/styles/themes/shared/theme.css" layer(theme);
@import "@khulnasoft/styles/themes/default" layer(theme);
/* OR specific theme files */
@import "@khulnasoft/styles/themes/default/index.css" layer(theme);
@import "@khulnasoft/styles/themes/default/variables.css" layer(theme);

/* Components (all components) */
@import "@khulnasoft/styles/components" layer(components);
/* OR specific component files */
@import "@khulnasoft/styles/components/index.css" layer(components);
@import "@khulnasoft/styles/components/button.css" layer(components);
@import "@khulnasoft/styles/components/accordion.css" layer(components);

/* Utilities (optional) */
@import "@khulnasoft/styles/utilities" layer(utilities);

/* Variants (optional) */
@import "@khulnasoft/styles/variants" layer(utilities);
```

> **Note**: Directory imports (e.g., `@khulnasoft/styles/components`) automatically resolve to their `index.css` file. Use explicit file paths (e.g., `@khulnasoft/styles/components/button.css`) to import individual component styles.

**Headless mode:** Build your own styles from scratch:

```css
@import "tailwindcss";
@import "@khulnasoft/styles/base/base.css";
...
PrismUI defines three types of variables in [`variables.css`](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/default/variables.css):
...
For a complete reference, see: [Colors Documentation](/docs/handbook/colors), [Default Theme Variables](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/default/variables.css), [Shared Theme Utilities](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/shared/theme.css)

**Tailwind theme bridge (`@theme inline`):**

[`themes/shared/theme.css`](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/shared/theme.css) maps semantic variables to Tailwind tokens (`--color-*`, `--radius-*`, `--ease-*`). Calculated colors reference underlying vars (e.g. `--surface-hover`, `--accent-soft`) from `variables.css` — they are not inlined with `color-mix()` in this file:
...
PrismUI applies a shared scrollbar style to component scroll areas (tables, popovers, drawers, and similar). Scrollbars use standard CSS properties (`scrollbar-width`, `scrollbar-color`, `scrollbar-gutter`) — no `::-webkit-scrollbar` overrides.

**Modes** — set `data-scrollbar` on `<html>`, a component root, or a scroll slot:

| Mode | `data-scrollbar` | Behavior |
|------|------------------|----------|
| PrismUI thin | *(unset)* or `thin` | Thin thumb from theme tokens |
...
<!-- Restore PrismUI scrollbars in one subtree -->
...
**Theme variables** — defined in light and dark theme blocks in [`variables.css`](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/default/variables.css):
...
**Custom overflow areas** — use the `scrollbar`, `scrollbar-thin`, `scrollbar-default`, or `scrollbar-none` utilities from `@khulnasoft/styles` on your own elements. See [Styling](/docs/handbook/styling) for class-based overrides.

> **Note**: Some components hide scrollbars by default (date picker popovers, color picker, secondary tabs). Nested scroll slots (e.g. the calendar year picker inside a date picker) keep PrismUI scrollbars because `scrollbar-none` only affects the element it is applied to, not descendants using `@apply scrollbar`.

## Vibrant Palette

By default, PrismUI uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast. If you prefer more saturated, vibrant soft foreground colors, add the `data-vibrant-palette` attribute to your root element:
