...
The complete theme definition can be found in ([variables.css](https://github.com/khulnasoft/heroui/blob/v3/packages/styles/themes/default/variables.css)). This theme automatically switches between light and dark modes based on the `class="dark"` or `data-theme="dark"` attributes.
