---
title: Colors
description: Color palette and theming system for PrismUI v3
icon: bucket-paint
links:
  themes: true
  tailwind: theme
...
PrismUI's color system is built around semantic intent, not visual abundance. Instead of exposing large raw palettes, the system defines a small, meaningful set of color roles that cover the majority of interface needs.

Most colors in the system are derived automatically from a limited number of base values. This allows PrismUI to maintain consistent contrast, hierarchy, and theming behavior while keeping the system easy to reason about and modify.
...
The complete theme definition can be found in ([variables.css](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/default/variables.css)). This theme automatically switches between light and dark modes based on the `class="dark"` or `data-theme="dark"` attributes.

<CollapsibleCode lang="css" code={`  @layer base {
    /* PrismUI Default Theme */
    :root {
      color-scheme: light;
