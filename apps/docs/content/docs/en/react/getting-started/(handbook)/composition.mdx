...
// Or import from @prismui/react (re-exports the same functions)
...
For more control and type safety, use variant functions to apply PrismUI styles to framework-specific components or custom elements. Variant functions are available from both `@khulnasoft/styles` (framework-agnostic) and `@prismui/react` (re-exports).
...
```tsx
import { Alert } from '@prismui/react';
...
```tsx
import { Alert, AlertTitle, AlertDescription } from '@prismui/react';
...
// Or named export
import { ButtonRoot } from '@prismui/react';
...
```tsx
import type { ButtonRootProps } from "@prismui/react";
...
```tsx
import type { ButtonRootProps, AvatarRootProps } from "@prismui/react";
...
```tsx
import { Button, Avatar } from "@prismui/react";
