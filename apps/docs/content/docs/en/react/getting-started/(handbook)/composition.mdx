...
PrismUI uses composition patterns to create flexible, customizable components. Change the rendered element, compose components together, and maintain full control over markup.

## Framework-Agnostic Styles

PrismUI's variant functions are available in the `@khulnasoft/styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use PrismUI's design system:

```tsx
// Import directly from @khulnasoft/styles (framework-agnostic)
import { buttonVariants } from '@khulnasoft/styles';

// Or import from @khulnasoft/react (re-exports the same functions)
import { buttonVariants } from '@khulnasoft/react';
```

Both imports work identically. Use `@khulnasoft/styles` when building for non-React frameworks or when you want to avoid pulling in React dependencies.

## Polymorphic Styling

Apply PrismUI styles to any element using variant functions or BEM classes. Extend component styles to framework components, native HTML elements, or custom components with full type safety.
...
When using a custom root element instead of PrismUI's Root component, child components cannot access context slots. You can manually pass `className` to child components using variant functions or BEM classes:
...
This approach works because PrismUI's variant functions and BEM classes can be applied to any element, giving you complete flexibility to style framework components, native elements, or custom components with PrismUI's design system.

## Direct Class Application

The simplest way to style links or other elements is to apply PrismUI's [BEM](https://getbem.com/) classes directly. This approach is straightforward and works with any framework or vanilla HTML.
...
This approach works because PrismUI uses [BEM](https://getbem.com/) classes that can be applied to any element. It's perfect when you don't need the component's interactive features (like `onPress` handlers) and just want the visual styling.

## Using Variant Functions

For more control and type safety, use variant functions to apply PrismUI styles to framework-specific components or custom elements. Variant functions are available from both `@khulnasoft/styles` (framework-agnostic) and `@khulnasoft/react` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@khulnasoft/styles`. Use them to apply PrismUI's design system to any element while maintaining type safety.

## Compound Components

PrismUI components are built as compound components—they export multiple parts that work together. Use them in three flexible ways:

**Option 1: Compound pattern (recommended)** — Use the main component directly without `.Root` suffix:

```tsx
import { Alert } from '@khulnasoft/react';
...
```tsx
import { Alert, AlertTitle, AlertDescription } from '@khulnasoft/react';
...
// Or named export
import { ButtonRoot } from '@khulnasoft/react';
...
```tsx
import { Link } from '@khulnasoft/react';
import { linkVariants, buttonVariants } from '@khulnasoft/styles';
...
Create your own components by composing PrismUI primitives:
...
```tsx
import type { ButtonRootProps } from "@khulnasoft/react";
import type { VariantProps } from "tailwind-variants";

import { Button } from "@khulnasoft/react";
import { buttonVariants, tv } from "@khulnasoft/styles";
...
```tsx
import type { ButtonRootProps, AvatarRootProps } from "@khulnasoft/react";
...
```tsx
import { Button, Avatar } from "@khulnasoft/react";
...
Since `@khulnasoft/styles` has no React dependencies, you can use it directly in any framework:
```vue
<script setup>
import { buttonVariants } from '@khulnasoft/styles';
