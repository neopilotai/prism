...
HeroUI's variant functions are available in the `@khulnasoft/styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use HeroUI's design system:
...
For more control and type safety, use variant functions to apply HeroUI styles to framework-specific components or custom elements. Variant functions are available from both `@khulnasoft/styles` (framework-agnostic) and `@khulnasoft/react` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@khulnasoft/styles`. Use them to apply HeroUI's design system to any element while maintaining type safety.
