---
title: Table
description: Migration guide for Table from PrismUI v2 to v3
---

<Callout type="info">
  Refer to the [v3 Table documentation](/docs/react/components/table) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from PrismUI v2.
</Callout>
...
```tsx
import { Table, TableHeader, TableColumn, TableBody, TableRow, TableCell } from "@khulnasoft/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Table, Checkbox } from "@khulnasoft/react";
