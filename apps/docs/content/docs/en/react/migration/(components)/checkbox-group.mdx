---
title: CheckboxGroup
description: Migration guide for CheckboxGroup from PrismUI v2 to v3
---

<Callout type="info">
  Refer to the [v3 CheckboxGroup documentation](/docs/react/components/checkbox-group) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from PrismUI v2.
</Callout>
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { useState } from "react";
    import { CheckboxGroup, Checkbox, Label } from "@khulnasoft/react";
