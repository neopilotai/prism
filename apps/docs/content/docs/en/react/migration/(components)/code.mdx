...
**v2:** `<Code>` component from `@prismui/react`  
...
```tsx
import { Code } from "@prismui/react";

export default function App() {
  return <Code>npm install @prismui/react</Code>;
}
```
    ...
    export default function App() {
      return (
        <div className="space-y-4">
          <p>
            Install PrismUI with <Code>npm install @prismui/react</Code>
          </p>
...
2. **Import Change**: Remove `import { Code } from "@prismui/react"`
...
1. **Remove Import**: Remove `Code` from `@prismui/react` imports
