---
title: ScrollShadow
description: Migration guide for ScrollShadow from PrismUI v2 to v3
---

<Callout type="info">
  Refer to the [v3 ScrollShadow documentation](/docs/react/components/scroll-shadow) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from PrismUI v2.
</Callout>
...
```tsx
import { ScrollShadow } from "@khulnasoft/scroll-shadow";
...
**v2:** Imported from `@khulnasoft/scroll-shadow` package  
**v3:** Imported from `@khulnasoft/react` unified package
...
- Update import from `@khulnasoft/scroll-shadow` to `@khulnasoft/react`
- Enhanced API with more configuration options
- Better shadow detection and visibility control
- Same orientation support (vertical / horizontal); import from `@khulnasoft/react`
