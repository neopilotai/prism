---
title: Snippet
description: Migration guide for Snippet from PrismUI v2 to v3
---

<Callout type="warning">
  The Snippet component has been **removed** in PrismUI v3. Use native HTML elements with Tailwind CSS classes and implement copy functionality manually using the Clipboard API.
</Callout>
...
**v2:** `<Snippet>` component from `@khulnasoft/react`  
...
```tsx
import { Snippet } from "@khulnasoft/react";
...
export default function App() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText("npm install @khulnasoft/react");
    setCopied(true);
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ...
    <Snippet symbol="$">
      {[
        "npm install @khulnasoft/react",
        "yarn add @khulnasoft/react",
        "pnpm add @khulnasoft/react"
      ]}
    </Snippet>
    ```
  </Tab>
...
2. **Import Change**: Remove `import { Snippet } from "@khulnasoft/react"`
...
1. **Remove Import**: Remove `Snippet` from `@khulnasoft/react` imports
