...
**v2:** `<Snippet>` component from `@prismui/react`  
...
```tsx
import { Snippet } from "@prismui/react";
...
export default function App() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText("npm install @prismui/react");
    setCopied(true);
...
2. **Import Change**: Remove `import { Snippet } from "@prismui/react"`
...
1. **Remove Import**: Remove `Snippet` from `@prismui/react` imports
