...
```tsx
import { Autocomplete, AutocompleteItem } from "@prismui/react";
...
```tsx
import { Autocomplete, Label, SearchField, ListBox, useFilter } from "@prismui/react";
...
The v3 Autocomplete uses the `useFilter` hook (from React Aria, re-exported by `@prismui/react`) to provide locale-aware filtering functions:

```tsx
import { useFilter } from "@prismui/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Header, Separator } from "@prismui/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { FieldError, Form } from "@prismui/react";
