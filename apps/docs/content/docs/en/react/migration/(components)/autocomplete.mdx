---
title: Autocomplete
description: Migration guide for Autocomplete from PrismUI v2 to v3
---

<Callout type="info">
  In v3, the v2 `Autocomplete` can be migrated to either the [v3 Autocomplete](/docs/react/components/autocomplete) or the [v3 ComboBox](/docs/react/components/combo-box), depending on your use case. See the section below on choosing the right component. This guide focuses on migrating from PrismUI v2.
</Callout>
...
```tsx
import { Autocomplete, AutocompleteItem } from "@khulnasoft/react";
...
```tsx
import { Autocomplete, Label, SearchField, ListBox, useFilter } from "@khulnasoft/react";
...
The v3 Autocomplete uses the `useFilter` hook (from React Aria, re-exported by `@khulnasoft/react`) to provide locale-aware filtering functions:

```tsx
import { useFilter } from "@khulnasoft/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Header, Separator } from "@khulnasoft/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { FieldError, Form } from "@khulnasoft/react";
