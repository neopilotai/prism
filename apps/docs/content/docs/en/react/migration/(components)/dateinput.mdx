---
title: DateInput
description: Migration guide for DateInput to DateField from PrismUI v2 to v3
---

<Callout type="info">
  Refer to the [v3 DateField documentation](/docs/react/components/date-field) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from PrismUI v2.
</Callout>
...
```tsx
import { DateInput } from "@khulnasoft/react";
...
```tsx
import { DateField, DateInputGroup, Label } from "@khulnasoft/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Description, FieldError, Label } from "@khulnasoft/react";
