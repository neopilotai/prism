...
**v2:** `<User>` component from `@prismui/react`  
...
```tsx
import { User } from "@prismui/react";
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { User, Link } from "@prismui/react";
...
2. **Import Change**: Remove `import { User } from "@prismui/react"`
...
1. **Remove Import**: Remove `User` from `@prismui/react` imports
