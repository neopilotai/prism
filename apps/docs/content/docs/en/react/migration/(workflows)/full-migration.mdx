...
description: Step-by-step guide for migrating PrismUI v2 to v3 using full migration approach
---

## Overview

Full migration is a structured approach to migrating from PrismUI v2 to v3. This method involves migrating all component code first, then switching dependencies, ensuring a clean transition.
...
**Note**: These dependency updates can be done before switching PrismUI (won't break project). However, the PrismUI package switch should only happen AFTER all component code is migrated.

#### Update PrismUI Packages
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm uninstall @khulnasoft/react @khulnasoft/theme
    npm install @khulnasoft/styles @khulnasoft/react
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm remove @khulnasoft/react @khulnasoft/theme
    pnpm add @khulnasoft/styles @khulnasoft/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn remove @khulnasoft/react @khulnasoft/theme
    yarn add @khulnasoft/styles @khulnasoft/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun remove @khulnasoft/react @khulnasoft/theme
...
```js
// tailwind.config.js
const {prismui} = require("@khulnasoft/react");

module.exports = {
  content: [
    "./src/**/*.{js,ts,jsx,tsx}",
    "./node_modules/@khulnasoft/theme/dist/**/*.{js,ts,jsx,tsx}",
  ],
  plugins: [prismui()],
  // ... other config
};
...
Remove the `prismui()` plugin from your Tailwind config. If you only used Tailwind for PrismUI and have no other customizations, you can remove `tailwind.config.js` entirely. Otherwise, keep the file but remove the PrismUI plugin configuration.
...
/* globals.css or main.css */
@import "tailwindcss";
@import "@khulnasoft/styles"; /* [!code highlight]*/
```

<Callout type="warning">
  Import order matters! Always import `tailwindcss` before `@khulnasoft/styles`.
</Callout>
...
<section id="step-3-remove-prismuiprovider">

### Step 3: Remove PrismUIProvider
...
PrismUI v2 provided component hooks (like `useSwitch`, `useInput`, `useCheckbox`, etc.) and utility hooks like `useDisclosure`. PrismUI v3 removes most component hooks in favor of compound components, and replaces `useDisclosure` with `useOverlayState`.
...
import {Button} from "@khulnasoft/button";
import {Card} from "@khulnasoft/card";
...
import {Button, type ButtonProps} from "@khulnasoft/react";

// Compound component types are properly exported
import {Checkbox, type CheckboxProps} from "@khulnasoft/react";
...
### Dependencies
- Update React to v19+
- Update PrismUI packages to v3 (after component migration)
- Remove Framer Motion
- Update Tailwind CSS to v4

### Configuration
- Remove `prismui()` plugin from Tailwind config
- Update CSS imports
- Remove `hero.ts` file (if exists)

### Application Code - Component Migration
- Remove `PrismUIProvider` wrapper
- Handle Provider props migration (router, locale, animations)
- Update all component imports to `@khulnasoft/react`
