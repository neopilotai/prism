...
```js
// tailwind.config.js
const {heroui} = require("@khulnasoft/react");
...
Remove the `heroui()` plugin from your Tailwind config. If you only used Tailwind for PrismUI and have no other customizations, you can remove `tailwind.config.js` entirely. Otherwise, keep the file but remove the PrismUI plugin configuration.
