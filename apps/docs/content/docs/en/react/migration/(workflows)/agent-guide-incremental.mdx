...
1. **Identify the strategy**: Check if project uses pnpm aliases (Strategy A) or component packages (Strategy B)
   - **Strategy A**: Look for aliases like `"@prismui-v3/react": "npm:@prismui/react@latest"` in package.json
   - **Strategy B**: Look for component-specific packages like `@khulnasoft/button`, `@khulnasoft/card` alongside `@prismui/react`

2. **Verify setup**: Ensure the coexistence setup is correct:
   - Strategy A: Both `@prismui/react` (v2) and `@prismui-v3/react` (v3 alias) are installed
   - Strategy B: `@prismui/react` (v3) and component packages like `@khulnasoft/button` (v2) are installed
...
2. **Update imports**
   - Change imports from `@prismui/react` to `@prismui-v3/react`
   - Example: `import {Button} from "@prismui/react"` → `import {Button} from "@prismui-v3/react"`
...
3. **Update imports**
   - Change imports from component package to `@prismui/react` (v3)
   - Example: `import {Card} from "@khulnasoft/card"` → `import {Card} from "@prismui/react"`
...
1. **Remove v2 dependencies**
   - Strategy A: Remove `@prismui/react`, `@khulnasoft/theme`, and aliases
...
2. **Update all imports**
   - Strategy A: Change `@prismui-v3/react` → `@prismui/react`
   - Strategy B: All imports should already be `@prismui/react` (v3)
...
- Projects using unified `@prismui/react` package (Strategy A can work but full migration may be simpler)
