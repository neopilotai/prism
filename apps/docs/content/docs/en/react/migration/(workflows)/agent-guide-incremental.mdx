...
1. **Identify the strategy**: Check if project uses pnpm aliases (Strategy A) or component packages (Strategy B)
   - **Strategy A**: Look for aliases like `"@heroui-v3/react": "npm:@khulnasoft/react@latest"` in package.json
...
2. **Verify setup**: Ensure the coexistence setup is correct:
   - Strategy A: Both `@khulnasoft/react` (v2) and `@heroui-v3/react` (v3 alias) are installed
...
2. **Update imports**
   - Change imports from `@khulnasoft/react` to `@heroui-v3/react`
   - Example: `import {Button} from "@khulnasoft/react"` → `import {Button} from "@heroui-v3/react"`
...
2. **Update all imports**
   - Strategy A: Change `@heroui-v3/react` → `@khulnasoft/react`
