...
- **Using `@prismui/react`**: Use Strategy A (pnpm aliases)
...
```json
{
  "dependencies": {
    ...
    "@prismui-v3/react": "npm:@prismui/react@latest",
    "@prismui-v3/styles": "npm:@khulnasoft/styles@latest"
  }
}
```

2. Import v2 components from `@prismui/react`:

```tsx
import {Button} from "@prismui/react"; // v2
```
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@prismui/react`, `@khulnasoft/theme`)
   - Remove aliases
   - Update all imports to `@prismui/react` (remove `-v3` suffix)
...
2. Import v3 components from `@prismui/react`:

```tsx
import {Button} from "@prismui/react"; // v3
```
...
2. **Update imports**
   - Strategy A: Change `@prismui/react` → `@prismui-v3/react`
   - Strategy B: Change component package → `@prismui/react`
...
1. **Remove v2 dependencies:**
   - Remove `@prismui/react` and `@khulnasoft/theme` (Strategy A)
...
2. **Update all imports:**
   - Change `@prismui-v3/react` → `@prismui/react` (Strategy A)
   - Change component package imports → `@prismui/react` (Strategy B)
