...
description: Step-by-step guide for migrating PrismUI v2 to v3 incrementally while keeping both versions working side-by-side
---

## Overview

Incremental migration with coexistence allows you to migrate from PrismUI v2 to v3 component-by-component while keeping your project functional throughout the migration process. This approach uses special setup strategies to allow both v2 and v3 components to work side-by-side.
...
- **Provider**: v2 requires `PrismUIProvider`, v3 doesn't - you may need conditional provider wrapping
...
- **Using `@khulnasoft/react`**: Use Strategy A (pnpm aliases)
- **Using component packages** (`@khulnasoft/button`, `@khulnasoft/card`, etc.): Use Strategy B (component packages)
...
{
  "dependencies": {
    "@khulnasoft/react": "2.8.6",
    "@khulnasoft/theme": "2.4.24",
    "@prismui-v3/react": "npm:@khulnasoft/react@latest",
    "@prismui-v3/styles": "npm:@khulnasoft/styles@latest"
  }
}
```

2. Import v2 components from `@khulnasoft/react`:

```tsx
import {Button} from "@khulnasoft/react"; // v2
```

3. Import v3 components from `@prismui-v3/react`:

```tsx
import {Button} from "@prismui-v3/react"; // v3
```

### Migration Process

1. **Migrate one component at a time:**
   - Update imports to use `@prismui-v3/react`
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@khulnasoft/react`, `@khulnasoft/theme`)
   - Remove aliases
   - Update all imports to `@khulnasoft/react` (remove `-v3` suffix)
...
{
  "dependencies": {
    "@khulnasoft/react": "latest", // v3
    "@khulnasoft/styles": "latest", // v3
    "@khulnasoft/button": "2.8.6", // v2
    "@khulnasoft/card": "2.8.6", // v2
    // ... other v2 component packages as needed
  }
}
```

2. Import v3 components from `@khulnasoft/react`:

```tsx
import {Button} from "@khulnasoft/react"; // v3
```

3. Import v2 components from component packages:

```tsx
import {Card} from "@khulnasoft/card"; // v2
```

### Migration Process

1. **Install v3 packages** (`@khulnasoft/react`, `@khulnasoft/styles`)
2. **Install v2 component packages** for components not yet migrated
3. **Migrate one component at a time:**
   - Remove the v2 component package from dependencies
   - Update imports to use `@khulnasoft/react` (v3)
   - Update component code to v3 API
   - Test the migrated component
   - For v2 components with no v3 counterpart (Code, Image, Navbar, Ripple, Snippet, Spacer, User), leave them in place. They will continue to work during coexistence.
...
<Callout type="warning">
  **Important:** Import order matters. Import `tailwindcss` first, then `@khulnasoft/styles`.
</Callout>
...
2. **Update imports**
   - Strategy A: Change `@khulnasoft/react` → `@prismui-v3/react`
   - Strategy B: Change component package → `@khulnasoft/react`
...
2. **Guide conflict resolution**
   - Ensure CSS import order: `tailwindcss` first, then `@khulnasoft/styles`
...
1. **Remove v2 dependencies:**
   - Remove `@khulnasoft/react` and `@khulnasoft/theme` (Strategy A)
   - Remove all `@khulnasoft/*` component packages (Strategy B)
...
2. **Update all imports:**
   - Change `@prismui-v3/react` → `@khulnasoft/react` (Strategy A)
   - Change component package imports → `@khulnasoft/react` (Strategy B)

3. **Update CSS:**
   - Remove v2 Tailwind plugin from config
   - Keep only `@import "@khulnasoft/styles";`
