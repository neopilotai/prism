...
description: Complete guide to styling changes and theming system migration from PrismUI v2 to v3
---

<Callout type="info">
  This guide covers all styling-related changes between PrismUI v2 and v3, including utility classes, component styles, theme system architecture, and visual differences. For component-specific API changes, see individual component migration guides.
</Callout>
...
PrismUI v3 introduces significant changes to the styling system:
...
PrismUI v2 provided custom text size utilities that mapped to CSS variables. v3 uses standard Tailwind text size classes.
...
v2 provided `.scrollbar-hide` and `.scrollbar-default` utilities. v3 now exposes standards-only scrollbar utilities from `@khulnasoft/styles`: `scrollbar`, `scrollbar-thin`, `scrollbar-default`, and `scrollbar-none`. Use `data-scrollbar="thin"`, `data-scrollbar="default"`, or `data-scrollbar="none"` on an ancestor for subtree-level control.
...
If you only use PrismUI, you can remove `tailwind.config.js` entirely. If you have custom Tailwind config, keep it but remove the PrismUI plugin.
...
v2 used the pattern `--prismui-{property}-{scale}` while v3 uses `--{property}` or `--color-{property}`.

**v2 CSS Variables:**

```css
--prismui-font-size-tiny: 0.75rem;
--prismui-font-size-small: 0.875rem;
--prismui-radius-small: 8px;
--prismui-radius-medium: 12px;
--prismui-border-width-medium: 2px;
--prismui-disabled-opacity: 0.5;
```
...
```css
--prismui-primary: 210 100% 50%;
--prismui-primary-50: 210 100% 95%;
--prismui-primary-100: 210 100% 90%;
/* ... more shades ... */
```
...
```css
--prismui-divider-weight: 1px;
--prismui-disabled-opacity: 0.5;
--prismui-hover-opacity: 0.8;
```
...
```css
--prismui-box-shadow-small: 0px 0px 5px 0px rgb(0 0 0 / 0.02), ...;
--prismui-box-shadow-medium: 0px 0px 15px 0px rgb(0 0 0 / 0.03), ...;
--prismui-box-shadow-large: 0px 0px 30px 0px rgb(0 0 0 / 0.04), ...;
```
