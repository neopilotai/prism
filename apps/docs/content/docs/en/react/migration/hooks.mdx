...
1. **Identify hook usage**: Search your codebase for imports from `@prismui/react` that include hook names (`useSwitch`, `useInput`, `useCheckbox`, `useRadio`, etc.)
...
```tsx
import { useSwitch } from "@prismui/react";
...
**Example:**
```tsx
import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, useDisclosure } from "@prismui/react";
...
**Example:**
```tsx
import { Modal, Button, useOverlayState } from "@prismui/react";
