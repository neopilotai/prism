---
title: Migration
description: Complete guide to migrate your PrismUI v2 application to v3
index: true
---

## For AI Assistants

Below are three approaches for AI assistants to access migration documentation. ***We recommend using the PrismUI Migration MCP server*** and leveraging its prompts and tools, but all options provide agents with the complete documentation.
...
| **Setup** | MCP config | Install command | `prism-cli` command |
...
- **Dependencies**: Update React to v19+, PrismUI packages to v3, Tailwind CSS to v4, remove Framer Motion
- **No Provider Required**: v3 removes the need for `PrismUIProvider`
...
- **Configuration**: Remove `prismui()` plugin from Tailwind config, update CSS imports, remove `hero.ts` file
...
PrismUI v2 and v3 cannot coexist in the same project without special setup. You have two migration approaches to choose from:
...
<Callout>
  **Component Development Status**: Components marked with 🔄 In Progress or 📋 Planned are still being developed. Check the [Roadmap](https://prismuiv3.featurebase.app/roadmap) for the task status. Guides for these components will be available once development is finished.
</Callout>
...
3. Check the [GitHub Discussions](https://github.com/khulnasoft/prismui/discussions)
