...
完整参考：[颜色文档](/docs/native/getting-started/colors)、[默认主题变量](https://github.com/khulnasoft/heroui-native/blob/main/src/styles/variables.css)、[共享主题工具](https://github.com/khulnasoft/heroui-native/blob/main/src/styles/theme.css)

**计算变量（Tailwind）：**

我们通过 Tailwind 的 `@theme` 指令自动生成按压态与圆角等计算变量，定义见 [theme.css](https://github.com/khulnasoft/heroui-native/blob/main/src/styles/theme.css)：
...
默认情况下，PrismUI Native 使用可读性更好的柔和前景色，即将语义色与前景色按比例混合，以在 soft 背景上获得更佳对比度。如果你偏好饱和度更高、更鲜亮的柔和前景色，可以在引入基础样式之后，额外导入可选的 `heroui-native/styles/vibrant` 样式：
