...
细粒度导入适合只用少量组件的场景，有助于控制包体。从 `/native` 整体导入会包含完整库，适合在应用中广泛使用多组件时采用。

**可用的细粒度入口：**

- `/native/provider` — Provider
- `/native/provider-raw` — 轻量 Provider（最小依赖起点）
- `/native/[component-name]` — 单个组件
- `/native/portal` — Portal 工具
- `/native/toast` — Toast Provider 与工具
- `/native/utils` — 工具函数
- `/native/hooks` — 自定义 Hooks

<Callout type="warning">
  **重要：** 为控制包体，请**始终**坚持使用细粒度导入策略。只要存在一处从 `/native` 的整体导入，就可能使上述优化失效。
</Callout>
