...
**相关 PR：** [#380](https://github.com/khulnasoft/prism/pull/380)
...
**相关 PR：** [#383](https://github.com/khulnasoft/prism/pull/383)
...
**相关 PR：** [#381](https://github.com/khulnasoft/prism/pull/381)

## 问题修复

本版本包含以下修复：

- **[Issue #363](https://github.com/khulnasoft/prism/issues/363)**：修复 Android 上 Button 的 outline 变体在通过条件属性切换到其他变体时边框仍残留的问题。Android 上的 React Native 在变体切换时有时会保留 `borderWidth`。除 `outline` 外的所有按钮变体现均包含显式 `border-0` 类，确保变体切换时将 `borderWidth` 重置为 `0`。

- **[Issue #357](https://github.com/khulnasoft/prism/issues/357)**：响应在 Card 上支持 `asChild` 以实现可按压卡片模式的诉求。PressableFeedback 与 Surface 上的新 `asChild` 插槽模式，使开发者可将按压与表面样式合并到单个子元素，无需额外包装节点。
...
**相关 PR：** [#384](https://github.com/khulnasoft/prism/pull/384)
