...
**相关 PR：** [#341](https://github.com/khulnasoft/heroui-native/pull/341)
...
**相关 PR：** [#343](https://github.com/khulnasoft/heroui-native/pull/343)
...
**相关 PR：** [#337](https://github.com/khulnasoft/heroui-native/pull/337)
...
**相关 PR：** [#347](https://github.com/khulnasoft/heroui-native/pull/347)
...
- **[Issue #330](https://github.com/khulnasoft/heroui-native/issues/330)**：修复 `Input` 在模块顶层无条件导入 `@gorhom/bottom-sheet` 的问题，即使 `isBottomSheetAware` 为 `false`。现通过可选的 `try/catch` 包装懒加载该包，不使用 Bottom Sheet 的项目无需再安装它。

- **[Issue #340](https://github.com/khulnasoft/heroui-native/issues/340)**：修复子菜单内容出现在同级子菜单触发器后方的问题。子菜单系统现按 ID 跟踪活动子菜单、强制单开，并应用正确的 z-index 分层（打开 `z-50`，关闭 `z-40`）。
