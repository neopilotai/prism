...
细粒度导入适合只用少数组件的场景；从 `/native` 总入口导入会包含完整库，适合全站大量使用。

**可用细粒度路径：**

- `/native/provider` — Provider
- `/native/[component-name]` — 各组件
- `/native/portal` — Portal 工具
- `/native/utils` — 工具函数
- `/native/hooks` — 自定义 Hooks

<Callout type="warning">
  **重要**：为控制包体，请在整个项目中**一致地**使用细粒度导入。只要存在一处从 `/native` 总入口的导入，摇树优化策略即可能失效。
</Callout>
