---
title: RC 1
description: Alert 组件、独立 Radio 组件、Select TriggerIndicator、PrismUINativeProviderRaw、disableFullWindowOverlay、styles 属性扩展、主题 surface 重构
github:
  pull: 284
---
...
RC 1 是 PrismUI Native 的首个候选发布（Release Candidate），表明库已接近生产可用。本版本引入以无障碍为先、带状态变体的 Alert 复合组件；将 Radio 抽为可双模式运行的独立组件；新增带动画的 Select.TriggerIndicator 子组件。还提供轻量 `PrismUINativeProviderRaw` 以优化包体、用于 iOS 调试的 `disableFullWindowOverlay`、六个组件统一的 `styles` 插槽式样式，以及用主题显式变量替代计算型 surface 色的主题重构。另含 BottomSheet 内 InputOTP、Toast 文字裁切与元素检查器兼容等关键修复。
...
**相关 PR：** [#284](https://github.com/khulnasoft/prismui-native/pull/284)
...
**相关 PR：** [#274](https://github.com/khulnasoft/prismui-native/pull/274)
...
**相关 PR：** [#229](https://github.com/khulnasoft/prismui-native/pull/229)
...
**相关 PR：** [#277](https://github.com/khulnasoft/prismui-native/pull/277)
...
**相关 PR：** [#281](https://github.com/khulnasoft/prismui-native/pull/281)
...
**相关 PR：** [#271](https://github.com/khulnasoft/prismui-native/pull/271)
...
使用 `PrismUINativeProvider` 时，Toast 通过 `config.toast` 传入该属性。

**相关 PR：** [#283](https://github.com/khulnasoft/prismui-native/pull/283)

### PrismUINativeProviderRaw

新增轻量提供者变体 `PrismUINativeProviderRaw`，不包含 `ToastProvider` 与 `PortalHost`，由使用方完全控制打包依赖。
...
**相关 PR：** [#285](https://github.com/khulnasoft/prismui-native/pull/285)
...
**相关 PR：** [#286](https://github.com/khulnasoft/prismui-native/pull/286)
...
- **[Issue #229](https://github.com/khulnasoft/prismui-native/issues/229)**：修复 BottomSheet 内 InputOTP 不可用。现可在 BottomSheet 遮罩内正常聚焦与输入，解决此前无法输入 OTP 的问题。

- **[Issue #265](https://github.com/khulnasoft/prismui-native/issues/265)**：修复 Toast 描述首字符被裁切。Toast 样式重构后以真实内边距与占位视图取代边框变通，任意堆叠配置下文字均可完整显示。

- **[Issue #272](https://github.com/khulnasoft/prismui-native/issues/272)**：修复 FullWindowOverlay 在 iOS 上阻挡 React Native 元素检查器。Portal 组件新增 `disableFullWindowOverlay`，开发期可将遮罩内容绘于主窗口以恢复检查器。
...
- [Provider](/docs/native/getting-started/provider) — PrismUINativeProviderRaw 与提供者层级
