...
PrismUI Native v1.0.7 让组件表面变得可扩展：组件现在接受 `background` 属性，并暴露配套的背景复合部件，从而可将表面后方的层替换为任意节点。本版本还新增 `GlassView` 层组件、用于 Dialog 与 Bottom Sheet 的 `blur` 遮罩变体，并将 `expo-blur` 作为可选的对等依赖。这些都是扩展点——默认主题不注册任何背景内容，因此渲染结果不变。
...
是否挂载默认层由 `--theme` CSS 变量决定，可使用 `useIsGlassTheme` 与 `useHasDefaultThemeBackground` 检测这一情况。PrismUI Native 附带的主题不注册任何背景内容，因此除非你自行注入，否则不会渲染任何内容；用于激活这些层的 `glass` 主题由 `prismui-native-pro/themes/glass` 提供。

**相关 PR：** [#457](https://github.com/khulnasoft/prismui-native/pull/457)
