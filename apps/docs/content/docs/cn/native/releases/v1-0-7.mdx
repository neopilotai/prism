...
是否挂载默认层由 `--theme` CSS 变量决定，可使用 `useIsGlassTheme` 与 `useHasDefaultThemeBackground` 检测这一情况。PrismUI Native 附带的主题不注册任何背景内容，因此除非你自行注入，否则不会渲染任何内容；用于激活这些层的 `glass` 主题由 `heroui-native-pro/themes/glass` 提供。

**相关 PR：** [#457](https://github.com/khulnasoft/heroui-native/pull/457)
