...
PrismUI Native v1.0.4 将排版基元 `Text` 重命名为 `Typography`，让出 React Native 自身的 `Text` 名称，同时更贴近语义化排版用法；既有的 `Text` 导出保留为弃用别名，可平滑升级。本版本还在 Alert、Avatar、Button、Chip、Toast 中调整了 soft 前景色令牌，使其在 soft 背景上具备更好的对比度；新增可选的 `prismui-native/styles/vibrant` 配色；为 Menu、Popover、Select 补充 iOS 原生模态偏移的处理说明；并将示例应用升级到 Expo 56 / React Native 0.85。
...
<Callout type="info">
  **无需迁移。** 既有的 `import { Text } from "prismui-native"` 通过弃用重导出仍然可用。请在未来某个大版本移除弃用别名之前，按你的节奏迁移到 `Typography`。
</Callout>

**相关 PR：** [#417](https://github.com/khulnasoft/prismui-native/pull/417)

### 鲜亮主题配色（可选）

新增可选的 `prismui-native/styles/vibrant` 样式表，适合希望在 soft 变体上保持更饱和品牌色的应用。它在保留调整后的柔和前景令牌带来的可读性改进的同时，仍为 soft 背景上的图标与文字保留较高的色彩饱和度。
...
在引入基础样式之后再引入 `prismui-native/styles/vibrant`，即可用更饱和的取值覆盖柔和前景令牌。无需修改任何组件代码即可启用。

**相关 PR：** [#420](https://github.com/khulnasoft/prismui-native/pull/420)
...
- 新增可选的 `prismui-native/styles/vibrant` 导出，为希望保留饱和品牌色的应用保留更鲜亮的 soft 变体外观
...
**相关 PR：** [#421](https://github.com/khulnasoft/prismui-native/pull/421)
...
`@gorhom/bottom-sheet` 的对等依赖范围由 `^5.2.8` 升级到 `^5.2.9`。如果你的应用使用 PrismUI Native 的 [BottomSheet](/docs/native/components/bottom-sheet)（或 `Menu` / `Popover` / `Select` 配合 `presentation="bottom-sheet"`），请确保安装兼容版本：
...
- 示例应用的 slug/bundle 重命名为 `prismui-native-oss`；调整 Android 上 `WithStateToggle` 的内边距
...
**相关 PR：** [#419](https://github.com/khulnasoft/prismui-native/pull/419)

## 问题修复

本版本包含以下修复：

- **[Issue #405](https://github.com/khulnasoft/prismui-native/issues/405)**：补充了在 React Navigation 原生模态中 `Menu` 渲染错位的处理方案。Menu 文档新增**原生模态（iOS）**章节，解释 Fabric / `FullWindowOverlay` 的坐标不一致，并演示如何用 `useSafeAreaInsets` 配合 `offset={insets.top}` 补偿。
- **[Issue #418](https://github.com/khulnasoft/prismui-native/issues/418)**：修复将示例应用升级到 Expo 56 后出现的 Hermes 崩溃（「Maximum call stack size exceeded」）及 `BottomSheet` 内容不可见的问题。示例的 `metro.config.js` 现已将对等依赖解析（`react`、`react-native`、`uniwind` 等）锁定到示例的本地 `node_modules`，防止从 workspace 根加载到重复副本。
