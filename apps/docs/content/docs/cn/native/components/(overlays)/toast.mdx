...
<NativeVideoPlayerView
  target="auto"
  srcLight="https://prismui-assets.nyc3.cdn.digitaloceanspaces.com/docs/native/components/videos/toast-docs-light.mp4"
  srcDark="https://prismui-assets.nyc3.cdn.digitaloceanspaces.com/docs/native/components/videos/toast-docs-dark.mp4"
/>
...
toast.show({
  variant: 'success',
  label: '套餐已升级',
  description: '可继续使用 PrismUI Chat',
...
更多示例见 [GitHub 仓库](<https://github.com/khulnasoft/prismui-native/blob/main/example/src/app/(home)/components/toast.tsx>)。

## 全局配置

通过 `PrismUINativeProvider` 的 `config` 全局配置 Toast；本地调用可覆盖默认值。
...
通过 `PrismUINativeProvider` 的 `config.toast` 进行全局配置时可用的属性。
...
访问 Toast 能力，必须在 `ToastProvider` 内使用（由 `PrismUINativeProvider` 提供）。
...
Toast 在 iOS 上使用 `FullWindowOverlay`。开发时若需使用 React Native 元素检查器，可在 `PrismUINativeProvider` 的 `config.toast` 中设置 `disableFullWindowOverlay={true}`。代价：Toast 将无法叠在原生系统模态之上。
