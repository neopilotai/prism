...
`Form` 渲染原生 `<form>` 元素，聚焦校验与提交。`@khulnasoft/styles` 中无专用 BEM 类——从全局 CSS 应用容器样式，控件级自定义请使用字段组件。
...
PrismUI 对在 `@khulnasoft/styles` 中提供样式的组件遵循 [BEM](https://getbem.com/) 方法论。
