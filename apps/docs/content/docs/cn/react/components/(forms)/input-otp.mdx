...
使用 `pattern` prop 限制输入字符。PrismUI 导出 `REGEXP_ONLY_CHARS`、`REGEXP_ONLY_DIGITS` 等常用模式。
...
InputOTP 组件使用以下 CSS 类（[查看源码样式](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/input-otp.css)）：
...
PrismUI 重新导出 input-otp 的常用正则模式以便使用：
