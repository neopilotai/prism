...
PrismUI 遵循 [BEM](https://getbem.com/) 方法论，确保组件变体与状态可复用且易于自定义。

### CSS 类

Dropdown 组件使用以下 CSS 类（[查看源码样式](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/dropdown.css)）：
...
#### 菜单类 [!toc]
- `.menu` - 菜单容器（[menu.css](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/menu.css)）
  - `[data-slot="separator"]` - 菜单内的分隔线元素

#### 菜单项类 [!toc]
- `.menu-item` - 菜单项容器（[menu-item.css](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/menu-item.css)）
...
#### 菜单分区类 [!toc]
- `.menu-section` - 菜单分区容器（[menu-section.css](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/components/menu-section.css)）
