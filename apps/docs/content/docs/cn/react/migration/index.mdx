---
title: 迁移
description: 将 PrismUI v2 应用迁移到 v3 的完整指南
index: true
---

## 面向 AI 助手

以下是 AI 助手获取迁移文档的三种方式。***我们推荐使用 PrismUI Migration MCP 服务器***，并充分利用其 prompt 与工具，不过其它方式也都能为 agent 提供完整的文档。

| 对比项 | MCP Server | Agent Skills | AGENTS.md |
|---------|-----------|--------------|-----------|
| **数据来源** | 远程端点 | 远程端点 | 本地文件 |
| **访问方式** | MCP 工具 | 脚本文件 | 读取本地文件 |
| **配置方式** | MCP 配置 | 安装命令 | `prism-cli` 命令 |
| **更新** | 实时 | 实时 | 手动 |
| **离线** | ❌ | ❌ | ✅ |
| **工具** | MCP 工具 + prompt | 脚本 | ❌ |
...
- **依赖项**：将 React 升级到 v19+、PrismUI 包升级到 v3、Tailwind CSS 升级到 v4，并移除 Framer Motion
- **无需 Provider**：v3 不再需要 `PrismUIProvider`
...
- **配置变更**：从 Tailwind 配置中移除 `prismui()` 插件，更新 CSS 导入，并删除 `hero.ts` 文件
...
如果不做特殊设置，PrismUI v2 与 v3 不能在同一个项目中共存。你可以选择以下两种迁移方式：
...
<Callout>
  **组件开发状态**：标有 🔄 进行中或 📋 计划中的组件仍在开发中。可以查看[路线图](https://prismuiv3.featurebase.app/roadmap)了解任务状态。这些组件的迁移指南将在开发完成后提供。
</Callout>
...
2. 查阅具体组件的迁移指南
3. 在 [GitHub Discussions](https://github.com/khulnasoft/prismui/discussions) 中查找或提问
