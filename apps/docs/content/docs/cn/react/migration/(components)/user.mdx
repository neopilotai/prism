---
title: User
description: User 从 PrismUI v2 到 v3 的迁移指南。
---

<Callout type="warning">
  PrismUI v3 中的 User 组件已被**移除**。请使用 Avatar、文本元素与 Tailwind CSS 类手动组合用户展示。
</Callout>
...
**v2：** 来自 `@khulnasoft/react` 的 `<User>` 组件  
...
2. **导入变更**：移除 `import { User } from "@khulnasoft/react"`
...
1. **移除导入**：从 `@khulnasoft/react` 导入中移除 `User`
