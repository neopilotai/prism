...
- **使用`@prismui/react`**：使用策略 A（pnpm 别名）
...
3. **迁移所有组件后，切换到仅 v3：**
   - 删除 v2 依赖项（`@prismui/react`, `@khulnasoft/theme`)
   - 删除别名
   - 将所有导入更新为`@prismui/react`（消除`-v3`后缀）
   - 完成样式迁移
...
1. **安装 v3 软件包** (`@prismui/react`, `@khulnasoft/styles`)
2. **为尚未迁移的组件安装 v2 组件包**
3. **一次迁移一个组件：**
   - 从依赖中删除 v2 组件包
   - 更新导入以使用`@prismui/react`(v3)
   - 将组件代码更新为 v3 API
   - 测试迁移的组件
   - 对于没有 v3 对位的 v2 组件（**Code**、**Image**、**Navbar**、**Ripple**、**Snippet**、**Spacer**、**User**），请将它们保留在原处，它们在共存期间仍会继续工作。
4. **继续，直到所有组件均已迁移**
...
2. **更新导入**
   - 策略A：改变`@prismui/react` → `@prismui-v3/react`
   - 策略 B：更改组件包 →`@prismui/react`
...
1. **删除 v2 依赖项：**
   - 消除`@prismui/react`和`@khulnasoft/theme`（策略A）
...
2. **更新所有导入：**
   - 改变`@prismui-v3/react` → `@prismui/react`（策略A）
   - 更改组件包导入 →`@prismui/react`（策略B）
