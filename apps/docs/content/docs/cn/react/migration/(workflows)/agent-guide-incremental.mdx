...
1. **确定策略**：检查项目是否使用pnpm别名（策略A）或组件包（策略B）
   - **策略 A**：寻找别名，例如`"@heroui-v3/react": "npm:@khulnasoft/react@latest"`在 package.json 中
   - **策略 B**：寻找特定于组件的包，例如`@khulnasoft/button`, `@khulnasoft/card`旁边`@khulnasoft/react`

2. **验证设置**：确保共存设置正确：
   - 策略A：两者兼而有之`@khulnasoft/react`（v2）和`@heroui-v3/react`（v3 别名）已安装
...
2. **更新导入**
   - 将导入从 `@khulnasoft/react` 改为 `@heroui-v3/react`
   - 例子：`import {Button} from "@khulnasoft/react"` → `import {Button} from "@heroui-v3/react"`
...
2. **更新所有导入**
   - 策略A：改变`@heroui-v3/react` → `@khulnasoft/react`
