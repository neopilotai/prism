...
1. **确定策略**：检查项目是否使用pnpm别名（策略A）或组件包（策略B）
   - **策略 A**：寻找别名，例如`"@prismui-v3/react": "npm:@prismui/react@latest"`在 package.json 中
   - **策略 B**：寻找特定于组件的包，例如`@khulnasoft/button`, `@khulnasoft/card`旁边`@prismui/react`

2. **验证设置**：确保共存设置正确：
   - 策略A：两者兼而有之`@prismui/react`（v2）和`@prismui-v3/react`（v3 别名）已安装
   - 策略B：`@prismui/react`(v3) 和组件包，例如`@khulnasoft/button`(v2) 已安装
...
2. **更新导入**
   - 将导入从 `@prismui/react` 改为 `@prismui-v3/react`
   - 例子：`import {Button} from "@prismui/react"` → `import {Button} from "@prismui-v3/react"`
...
3. **更新导入**
   - 将组件包中的导入更改为`@prismui/react`(v3)
   - 例子：`import {Card} from "@khulnasoft/card"` → `import {Card} from "@prismui/react"`
...
- 可以接受临时破坏状态的项目
- 喜欢一次性迁移的团队
- 项目使用统一`@prismui/react`包（策略 A 可以工作，但完全迁移可能更简单）
