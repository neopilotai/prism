...
Table 与 ListBox 现在可以借助 React Aria 的 `Virtualizer` 来支持大数据集的虚拟化渲染。`Virtualizer`、`TableLayout` 和 `ListLayout` 都已从 `@prismui/react` 重新导出。
...
`@prismui/react` 现在支持按组件的子路径入口，方便更明确地按需引入（[#6301](https://github.com/khulnasoft/prismui/pull/6301)）：
