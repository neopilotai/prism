...
将 `@prismui/react` 和 `@khulnasoft/styles` 及其对等依赖安装到你的项目中。若它们已安装，则该命令不会执行任何操作。
...
将 `@prismui/react` 和 `@khulnasoft/styles` 及其对等依赖升级到最新版本。
...
从你的项目中卸载 `@prismui/react` 和 `@khulnasoft/styles`。对等依赖不会被卸载。
...
列出已安装的 PrismUI 包（`@prismui/react`、`@khulnasoft/styles`）。
...
检查项目中存在的问题。
- 检查 `@prismui/react` 和 `@khulnasoft/styles` 是否已安装
