...
HeroUI 的主题系统构建于 [Tailwind CSS v4](https://tailwindcss.com/docs/theme) 的主题之上。当你导入 `@khulnasoft/styles` 时，它会使用 Tailwind 的内置调色板，将其映射到语义化变量，自动在浅色和深色主题之间切换，并使用 CSS 层和 `@theme` 指令进行组织。
...
HeroUI 在 [`variables.css`](https://github.com/khulnasoft/heroui/blob/v3/packages/styles/themes/default/variables.css) 中定义了三种类型的变量：
