---
title: 颜色
description: PrismUI v3 的调色板与主题系统
icon: bucket-paint
links:
  themes: true
  tailwind: theme
...
PrismUI 的颜色体系围绕语义意图构建，而非堆砌视觉色板。系统不会暴露庞大的原始色表，而是定义一小套有意义的色彩角色，覆盖绝大多数界面需求。
...
完整主题定义见仓库中的 [variables.css](https://github.com/khulnasoft/prismui/blob/v3/packages/styles/themes/default/variables.css)。该主题会根据 `class="dark"` 或 `data-theme="dark"` 属性在明暗模式间自动切换。
