...
HeroUI 的变体函数位于 `@khulnasoft/styles` 包中，可以独立于 React 使用。这使得 Vue、Svelte 等其他框架也能使用 HeroUI 的设计系统：
...
如需更多控制和类型安全，可以使用变体函数将 HeroUI 样式应用到特定框架的组件或自定义元素上。`@khulnasoft/styles`（与框架无关）和 `@khulnasoft/react`（重新导出）都提供了变体函数。
...
**可用的变体函数：** 每个组件都从 `@khulnasoft/styles` 导出其变体函数（`buttonVariants`、`chipVariants`、`linkVariants`、`spinnerVariants` 等）。使用它们可以在保持类型安全的同时，将 HeroUI 的设计系统应用到任何元素上。
