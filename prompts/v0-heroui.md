...
- All PrismUI components work in both Server Components and Client Components.
- For interactive state (e.g. controlled modals, form handlers), mark the file `"use client"`.
- Static rendering of PrismUI components (cards, badges, layout) works in Server Components without `"use client"`.
...
PrismUI works with Tailwind CSS v4 classes via `className`:
...
1. **Never** wrap in a PrismUI Provider — not needed in v3.
2. **Never** import from `@nextui-org/*` — that is the old v2 library.
3. **Never** use Tailwind v3 config (`tailwind.config.js`). PrismUI v3 requires Tailwind CSS v4 with CSS-based configuration.
