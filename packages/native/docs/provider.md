...
```tsx
import { PrismUINativeProvider } from '/native';
import type { PrismUINativeConfig } from '/native';
...
Use `PrismUINativeProviderRaw` when you want full control over which dependencies are included in your bundle. With the raw provider imported from `/native/provider-raw`, the following dependencies are optional and only required if you use the corresponding components:
...
```tsx
import { PrismUINativeProviderRaw } from '/native/provider-raw';
import { PortalHost } from '/native/portal';
import { ToastProvider } from '/native/toast';
...
```tsx
import { PrismUINativeProvider, type PrismUINativeConfig } from '/native';
