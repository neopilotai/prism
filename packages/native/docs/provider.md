...
```tsx
import { PrismUINativeProvider } from 'heroui-native';
import type { PrismUINativeConfig } from 'heroui-native';
...
Use `PrismUINativeProviderRaw` when you want full control over which dependencies are included in your bundle. With the raw provider imported from `heroui-native/provider-raw`, the following dependencies are optional and only required if you use the corresponding components:
...
```tsx
import { PrismUINativeProviderRaw } from 'heroui-native/provider-raw';
...
```tsx
import { PrismUINativeProvider, type PrismUINativeConfig } from 'heroui-native';
