...
Configure PrismUI Native provider with text, text input, animation, toast, and layout direction settings

## Overview

The provider serves as the main entry point for PrismUI Native, wrapping your application with essential contexts and configurations:
...
- **Text Configuration**: Global text component settings for consistency across all PrismUI components
- **Text Input Configuration**: Global text input settings for consistency across all PrismUI input components
...
Global settings for all Text components within PrismUI Native. These props are carefully selected to include only those that make sense to configure globally across all Text components in the application:

```tsx
import { PrismUINativeProvider } from 'prismui-native';
import type { PrismUINativeConfig } from 'prismui-native';
...
export default function App() {
  return (
    <PrismUINativeProvider config={config}>
      {/* Your app content */}
    </PrismUINativeProvider>
  );
}
...
Global settings for all TextInput-based components within PrismUI Native (e.g. Input, TextArea, SearchField, InputGroup, InputOTP). These props are carefully selected to include only those that make sense to configure globally across all inputs in the application:
...
Tell PrismUI Native components which layout direction they render in:
...
The `PrismUINativeProvider` internally composes multiple providers:
...
`PrismUINativeProviderRaw` is a lightweight variant of `PrismUINativeProvider` designed for bundle optimization. It excludes `ToastProvider` and `PortalHost`, giving you a bare minimum starting point where you only install and add what you actually need. Its `PrismUINativeConfigRaw` config accepts the same options as `PrismUINativeConfig` except `toast`, so `textProps`, `textInputProps`, `animation`, `devInfo`, and `isRTL` all behave identically.

### When to Use

Use `PrismUINativeProviderRaw` when you want full control over which dependencies are included in your bundle. With the raw provider imported from `prismui-native/provider-raw`, the following dependencies are optional and only required if you use the corresponding components:
...
import {
  PrismUINativeProviderRaw,
  type PrismUINativeConfigRaw,
} from 'prismui-native/provider-raw';
import { GestureHandlerRootView } from 'react-native-gesture-handler';

const config: PrismUINativeConfigRaw = {
  textProps: {
    maxFontSizeMultiplier: 1.5,
  },
};
...
export default function App() {
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <PrismUINativeProviderRaw config={config}>
        {/* Your app content */}
      </PrismUINativeProviderRaw>
    </GestureHandlerRootView>
  );
...
```tsx
import { PrismUINativeProviderRaw } from 'prismui-native/provider-raw';
import { PortalHost } from 'prismui-native/portal';
import { ToastProvider } from 'prismui-native/toast';
...
Always use a single `PrismUINativeProvider` at the root of your app. Don't nest multiple providers:

```tsx
// ❌ Bad
<PrismUINativeProvider>
  <SomeComponent>
    <PrismUINativeProvider> {/* Don't do this */}
      <AnotherComponent />
    </PrismUINativeProvider>
  </SomeComponent>
</PrismUINativeProvider>
...
function App() {
  return (
    <PrismUINativeProvider config={config}>{/* ... */}</PrismUINativeProvider>
  );
}
```
...
```tsx
import { PrismUINativeProvider, type PrismUINativeConfig } from 'prismui-native';
