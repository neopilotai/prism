 ...
 * Toast Queue
 * --------------------------------------------------------------------------------------------- */
/** The underlying react-stately queue passed to `ToastRegion` (not the PrismUI `ToastQueue` wrapper). */
...
export interface PrismUIToastOptions {
...
function createToastFunction(queue: ToastQueue<ToastContentValue>) {
  const toastFn = (message: ReactNode, options?: PrismUIToastOptions): string => {
  ...
  // Variant methods
  toastFn.success = (message: ReactNode, options?: Omit<PrismUIToastOptions, "variant">): string => {
    return toastFn(message, {...options, variant: "success"});
  };

  toastFn.danger = (message: ReactNode, options?: Omit<PrismUIToastOptions, "variant">): string => {
    return toastFn(message, {...options, variant: "danger"});
  };

  toastFn.info = (message: ReactNode, options?: Omit<PrismUIToastOptions, "variant">): string => {
    return toastFn(message, {...options, variant: "accent"});
  };

  toastFn.warning = (message: ReactNode, options?: Omit<PrismUIToastOptions, "variant">): string => {
