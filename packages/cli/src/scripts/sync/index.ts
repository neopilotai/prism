...
export function syncDocs() {
  ...
  const matchDocs = docs.match(/(?<=Usage: prismui \[command]\n\n)[\W\w]+(?=## Documentation)/)?.[0];

  const targetPath = resolver('prismui/apps/docs/content/docs/api-references/cli-api.mdx');
  const targetDocs = readFileSync(targetPath, 'utf-8');
  const replaceTargetDocs = targetDocs.replace(/(?<=Usage: prismui \[command])[\W\w]+/, '');
...
function syncApiRoutes() {
  const targetPath = resolver('prismui/apps/docs/config/routes.json');
