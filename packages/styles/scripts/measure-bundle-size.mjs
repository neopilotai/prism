...
async function main() {
  ...
  console.log("PrismUI CSS Bundle Size Measurement");
  console.log("=".repeat(60));
  console.log();
  ...
  if (compareOnly && previous) {
    ...
    console.log("Minified (prismui.min.css):", formatBytes(previous.minified.bytes).formatted);
    console.log();
  ...
  const minifiedPath = path.join(distDir, "prismui.min.css");
  ...
  console.log(
    `Minified (prismui.min.css):  ${result.minified.formatted}${previous ? showComparison(minifiedSize, previous.minified.bytes, "minified") : ""}`,
  );
  console.log(
