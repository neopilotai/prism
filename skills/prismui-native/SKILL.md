...
# PrismUI Native Development Guide

PrismUI Native is a component library built on **Uniwind (Tailwind CSS for React Native)** and **React Native**, providing accessible, customizable UI components for mobile applications.
...
**This guide is for PrismUI Native ONLY.** Do NOT apply PrismUI React (web) patterns — the package, styling engine, and color format all differ:
...
1. **Uniwind is Required** - PrismUI Native uses Uniwind (Tailwind CSS for React Native)
2. **PrismUINativeProvider Required** - Wrap your app with `PrismUINativeProvider`
...
PrismUI Native uses **compound component patterns**. Each component has subcomponents accessed via dot notation.
...
PrismUI Native uses CSS variables via Tailwind/Uniwind for theming. Theme colors are defined in `global.css`:
