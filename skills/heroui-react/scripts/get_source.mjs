#!/usr/bin/env node
/**
 * Get React/TypeScript source code implementation for PrismUI v3 components.
 *
 * Usage:
...
const GITHUB_RAW_BASE = "https://raw.githubusercontent.com/khulnasoft/heroui/refs/heads/v3";
  ...
  for (const path of patterns) {
    ...
    try {
      const response = await fetch(url, {
        headers: {"User-Agent": "PrismUI-Skill/1.0"},
        signal: AbortSignal.timeout(30000),
      });
      ...
      if (response.ok) {
        ...
        return {
          component,
          filePath: path,
          githubUrl: `https://github.com/khulnasoft/heroui/blob/v3/${path}`,
          source: "fallback",
          sourceCode: content,
        };
      }
